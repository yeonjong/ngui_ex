...
public class AttackPartyEditPanel : PanelBase {
	...
	void OnEnable() {
		...
		charSet = mainParty.m_charSetList [selectedParty];
		form = mainParty.m_formList [selectedParty];
	...
	public void ChangeFormation(int formNum) {
		...
		GlobalApp.Inst.userData.GetUser().SetFormation (type, selectedParty, formNum);
