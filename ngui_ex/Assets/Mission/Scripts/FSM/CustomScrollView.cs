...
{
	static public BetterList<CustomScrollView> list = new BetterList<CustomScrollView>();
	...
	{
		/* yeonjong_begin */
		Vector2 mapBounds = bounds.max;
		Vector2 panelSize = mPanel.GetViewSize ();
		limitX = mapBounds.x - (panelSize.x * 0.5f);
		limitY = mapBounds.y - (panelSize.y * 0.5f);
		//Debug.Log (limitX);
		//Debug.Log (limitY);
		/* yeonjong_end */
	...
	{
		/* yeonjong_begin */
		x = mTrans.localPosition.x + relative.x;
		y = mTrans.localPosition.y + relative.y;
		x = Mathf.Abs (x);
		y = Mathf.Abs (y);
		if (x > limitX) {
			// fix x to limitX
			relative.x *= 0.075f;//= 0f;
		}
		if (y > limitY) {
			// fix y to limitY
			relative.y *= 0.15f; //= 0f;
